...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        for i_trial in range(N_repeat):
            ...
            # 0.0.0. z_r_tilde_mul for surviving
            # comm.Barrier()
            z_r_tilde_mul_array_surviving = [0] * len(surviving_users_indexes_actual)  #len(surviving_users_indexes)
            array_idx = 0
            for i in surviving_users_indexes_actual: #surviving_users_indexes:
                rx_rank = i + 1
                z_r_tilde_mul_array_surviving[array_idx] = comm.recv(source=rx_rank)
                array_idx += 1

            # 0.0.1. z_r_tilde_mul for dropped
            # comm.Barrier()
            z_r_tilde_mul_array_dropped = [0] * len(surviving_users_indexes_actual) #len(surviving_users_indexes)
            array_idx = 0
            for i in surviving_users_indexes_actual: #surviving_users_indexes:
                rx_rank = i + 1
                z_r_tilde_mul_array_dropped[array_idx] = comm.recv(source=rx_rank)
                array_idx += 1
            ...
            #decode the other mask: z_r
            dec_z_r = LightVeriFL_dec_EC(z_r_tilde_mul_array_surviving, alpha_s, beta_s[surviving_users_indexes_actual], P256)
            dec_z_r_minus = (-1 * dec_z_r)

            res_surviving = (hz_mul_surviving + dec_z_minus)
            noise_surviving = (noise_mul_surviving + dec_z_r_minus)
            ...
            #decode the other mask: z_r
            dec_z_r = LightVeriFL_dec_EC(z_r_tilde_mul_array_dropped, alpha_s, beta_s[surviving_users_indexes_actual], P256)
            dec_z_r_minus = (-1 * dec_z_r)

            res_dropped = (hz_mul_dropped + dec_z_minus)
            noise_dropped = (noise_mul_dropped + dec_z_r_minus)
        ...
        for i_trial in range(N_repeat):
            ...
            z_r = generate_point_EC() #another mask for the random point of the commitment
            ...
            z_r_tilde_array = LightVeriFL_enc_EC(z_r, n_array, alpha_s, beta_s, P256)
            ...
            z_tilde_buffer = [0] * N
            z_r_tilde_buffer = [0] * N

            z_tilde_buffer[rank - 1] = z_tilde_array[rank - 1]
            z_r_tilde_buffer[rank - 1] = z_r_tilde_array[rank - 1]
            ...
            for j in range(len(tx_dest)):
                ...
                tx_data = z_r_tilde_array[bf_addr]
                comm.send(tx_data, dest=tx_rank)
            ...
            for i in range(len(rx_source)):
                bf_addr = rx_source[i]
                rx_rank = rx_source[i] + 1
                z_r_tilde_buffer[bf_addr] = comm.recv(source=rx_rank)
            ...
            Pedersen_noise_z = (Pedersen_noise + z_r)  # mask the Pedersen noise
            ...
            if rank in surviving_users_indexes_actual + 1: #surviving_users_indexes + 1:
                surviving_users_indexes_list = surviving_users_indexes.tolist()
                z_r_tilde_buffer_surviving = [z_r_tilde_buffer[i] for i in surviving_users_indexes_list]
                z_r_tilde_mul_surviving = PI_addEC(z_r_tilde_buffer_surviving)
                comm.send(z_r_tilde_mul_surviving, dest=0)

            # 0.0.1. Send z_tilde_mul for dropped users
            if rank in surviving_users_indexes_actual + 1: #surviving_users_indexes + 1:
                z_r_tilde_buffer_dropped = [z_r_tilde_buffer[i] for i in dropped_users_indexes_list]
                z_r_tilde_mul_dropped = PI_addEC(z_r_tilde_buffer_dropped)  # for the dropped users
                comm.send(z_r_tilde_mul_dropped, dest=0)
