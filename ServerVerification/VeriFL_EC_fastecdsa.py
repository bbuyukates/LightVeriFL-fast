...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        for i_trial in range(N_repeat):
            ...
            t_VeriRound1_commSS = time.time() - t1_verification
            ...
            t_VeriRound1_decEC = time.time() - t1_verification - t_VeriRound1_commSS
            ...
            comm.Barrier()
            t_VeriRound2_pederson = time.time() - t2_verification

            comm.Barrier()
            t_VeriRound2_genhash = time.time() - t2_verification - t_VeriRound2_pederson
            ...
            print(f"t_VeriRound1_commSS={t_VeriRound1_commSS}, t_VeriRound1_decEC = {t_VeriRound1_decEC}\n")
            print(f"t_VeriRound2_pederson={t_VeriRound2_pederson}, t_VeriRound2_genhash={t_VeriRound2_genhash}, t2_verification_remain = {t_VeriRound2-t_VeriRound2_pederson-t_VeriRound2_genhash}")
        ...
        pickle.dump(time_out, open('./results/VeriFL_fast_N' + str(N) +'_U' + str(U)+ '_d' + str(d), 'wb'), -1)
        ...
        for i_trial in range(N_repeat):
            ...
            Verification Round 2 (BatchChecking)
                ...
                - (TODO) Implement amortized-verification 
            '''
            # We do not need to worry about the amortized verification for now as they use batch size = 1 in simulations
            ...
            # Clients also verify the surviving users' hashes by checking the commitments

                for i in range(len(surviving_users_indexes)):
                    # reconstruct the commitment of ith dropped user
                    tmp_hash2 = hash_array[i]
                    tmp_noise2 = noise_array[i]
                    comm_reconstructed = generate_Pedersen_commitment(tmp_hash2, Pedersen_g, Pedersen_l, tmp_noise2)
                    if comm_reconstructed.x == comm_array[surviving_users_indexes[i]].x and comm_reconstructed.y == comm_array[surviving_users_indexes[i]].y:
                        #print(f"Verification @ rank {rank}: Server reconstruction is correct. Proceed.")
                        pass
                    else:
                        print(f"Verification @ rank {rank}: Forged reconstruction at the server. Process terminated.")
            ...
            if rank in surviving_users_indexes + 1:
                # Each client computes the aggregate hash
                agg_hash = generate_hash(agg_grad, alpha, d)

            comm.Barrier()

            if rank in surviving_users_indexes + 1:
                hashes_summed = Point(0, 0, curve=None)  # None

                # add surviving users' hashes
                for i in range(len(surviving_users_indexes)):
                    temp_element = hash_array[i]
                    temp_element = Point(temp_element.x, temp_element.y, curve=P256)
                    hashes_summed = (temp_element + hashes_summed)
                    hashes_summed = Point(hashes_summed.x, hashes_summed.y, curve=P256)

                # add dropped users' hashes
                for i in range(len(dropped_users_indexes)):
                    temp_element2 = h_dropped_buffer[i]
                    temp_element2 = Point(temp_element2.x, temp_element2.y, curve=P256)
                    hashes_summed = (temp_element2 + hashes_summed)
                    hashes_summed = Point(hashes_summed.x, hashes_summed.y, curve=P256)

                if agg_hash.x == hashes_summed.x and agg_hash.y == hashes_summed.y:
