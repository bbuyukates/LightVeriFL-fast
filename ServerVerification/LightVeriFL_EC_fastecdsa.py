...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        for i_trial in range(N_repeat):
            ...
            dec_z = LightVeriFL_dec_EC(z_tilde_mul_array_surviving, alpha_s, beta_s[surviving_users_indexes_actual],
                                       P256)
            dec_z_minus = -1 * dec_z
            ...
            dec_z = LightVeriFL_dec_EC(z_tilde_mul_array_dropped, alpha_s, beta_s[surviving_users_indexes_actual],
                                       P256)
            dec_z_minus = -1 * dec_z
            ...
            # each client generates hash of grad_aggr

            comm.Barrier()
            t1_verification_genhash = time.time() - t1_verification

            # each client checked the aggregation is performed correctly.
            ...
            print(f"t1_verification_genhash={t1_verification_genhash}")
        ...
        pickle.dump(time_out, open('./results/LightVeriFL_fast_N' + str(N) + '_U' + str(U) + '_d' + str(d), 'wb'), -1)
        ...
        for i_trial in range(N_repeat):
            ...
            #print('in aggregation hash generation', t_hash_gen)
            ...
            if rank in surviving_users_indexes + 1:
                ...
                for i in range(N):
                    temp_comm = comm_array[i]
                    temp_comm = Point(temp_comm.x, temp_comm.y, curve=P256)
                    commitments_summed = temp_comm + commitments_summed  # comm_array[i][:] + commitments_summed
            ...
            if rank in surviving_users_indexes + 1:
                if h_agg.x == result.x and h_agg.y == result.y:
